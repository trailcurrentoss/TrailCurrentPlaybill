/* DVB / ATSC tuner service — Hauppauge WinTV-dualHD (model 1595) and any
   other in-tree linux-dvb device. The kernel driver (`dvb_usb_cxusb` family)
   exposes adapters as /dev/dvb/adapterN/{frontend0, demux0, dvr0}.

   Public surface (UI-agnostic — designed so the same calls can be exposed
   over IPC to the local renderer AND, later, over HTTP to the Headwaters
   PWA for remote control + restream):
