...
echo "    5. Power on. Plymouth → GDM auto-login (~30s) as trailcurrent"
echo "       (default password trailcurrent — change via Settings → Users after login)"
echo "    6. First login: GNOME Initial Setup wizard prompts for timezone,"
echo "       keyboard, WiFi, and privacy. Sentinel ~/.config/gnome-initial-setup-done"
echo "       suppresses it on every subsequent boot."
